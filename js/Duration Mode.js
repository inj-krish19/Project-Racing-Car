...
var score = 0, highestScoreDurationMode = 0, gameStatus = 1;
...
class Blocks {
    ...
    draw() {
        ...
        for (let i = 1; i <= this.hei / 100; i++) {

            c.fillStyle = "white";
            c.font = "100px Calibary";
            c.fillText("+", this.position.x + (this.wid / 4), this.position.y + (i * 100));
        }

        // console.log(this.position.x, this.position.y, this.wid, this.hei);
    }
...
var blockY = 0, blockH = 0, TBN = -1;

for (let i = 1; i <= 50; i++) {

    choice = Math.floor(Math.random() * cars_x.length);
    blockY = i * -1000;
    blockH = 100 * (Math.ceil(Math.random() * 5));

    if (blockH < 200) {
        blockH = 200;
    }
    tempD = 100 * (Math.ceil(Math.random() * 5));
    if (tempD < 200) {
        tempD = 200;
    }
    ...
    blockY -= tempD;
    if (i != 0) {
        timeBlocks.push(new Blocks(cars_x[choice], blockY, blockH));
    } else {
        timeBlocks.push(new Blocks(cars_x[choice], blockY - (2 * timeBlocks[i - 1].hei), blockH));
    }
    timeBlocks[i - 1].draw();
}
...
function increaseDuration() {
    ...
    for (j = 0; j < timeBlocks.length; j++) {
        if (
            ((car[0].position.y - timeBlocks[j].position.y < timeBlocks[0].hei) &&
                (car[0].position.y - timeBlocks[j].position.y > (-1 * timeBlocks[0].hei))) &&
            ((car[0].position.x - timeBlocks[j].position.x) == 0 ||
                ((car[0].position.x - timeBlocks[j].position.x) > 0 &&
                    (car[0].position.x - timeBlocks[j].position.x) < 10))
        ) {
...
function DurationIncreased() {

    document.getElementById('timeSeconds').style.color = "green";
    Duration += Math.ceil(timeBlocks[TBN].hei);
...
function setScores() {
    if (sessionStorage.highestScoreDurationMode > 0) {

    } else {
        sessionStorage.highestScoreDurationMode = 0;
    }

    if (localStorage.highestScoreDurationMode > 0) {

    } else {
        localStorage.highestScoreDurationMode = 0;
    }
}

setScores();

function givehighestScoreDurationMode() {
    highestScoreDurationMode = score;
    if (sessionStorage.highestScoreDurationMode < highestScoreDurationMode) {
        sessionStorage.highestScoreDurationMode = highestScoreDurationMode;
    }
    if (localStorage.highestScoreDurationMode < highestScoreDurationMode) {
        localStorage.highestScoreDurationMode = highestScoreDurationMode;
    }
}
...
function makeCountDown() {
    ...
    if (seconds > -1000) {
        ...
        setInterval(DurationDecrease, 1000);
    }
...
function colorChanger() {
    ...
    if (Duration <= 5000) {
        document.getElementById('timeSeconds').style.color = "red";
    }
    else if (Duration <= 15000) {
        document.getElementById('timeSeconds').style.color = "yellow";
    } else if (Duration >= 45000) {
        document.getElementById('timeSeconds').style.color = "purple";
    } else {
        document.getElementById('timeSeconds').style.color = "blue";
    }
...
function timerStyle() {
    ...
    if (Duration > 0) {
        document.getElementById('timeSeconds').innerHTML = Duration / 1000;
    } else {
        document.getElementById('timeSeconds').style.marginTop = "-65px";
        document.getElementById('timeSeconds').style.fontSize = "25px";
        document.getElementById('timeSeconds').innerHTML = "Time Over";
        allPause();
        document.getElementById('result').innerHTML = 'Time Ruined';
...
for (let index = 0; index < timeBlocks.length; index++) {
    timeBlocks[index].draw();
}

function animate() {

    document.getElementById('scoreSession').innerHTML = "Sessions' Highest Score : " + sessionStorage.highestScoreDurationMode;
    document.getElementById('scoreAllTime').innerHTML = "Best Score : " + localStorage.highestScoreDurationMode;
    if (score > 0) {
    ...
    if (car[car.length - 1].position.y - width > 0) {
        for (let i = 1; i < car.length; i++) {
    ...
    if (timeBlocks[timeBlocks.length - 1].position.y > 0 && timeBlocks[timeBlocks.length - 1].position.y < 1000) {
        for (let i = 1; i < timeBlocks.length; i++) {
            ...
            blockY -= tempD;
            timeBlocks[i - 1].position.x = cars_x[choice];

            if (i != 0) {
                timeBlocks[i - 1].position.y = 0 + blockY;
            } else {
                timeBlocks[i - 1].position.y = 0 + blockY - (2 * timeBlocks[i - 1].hei);
            }
