...
var score = 0, highestScoreCasualMode = 0, gameStatus = 1;
...
function ImageValidation(i) {
    ...
    if (counter === 0) { temporary = ima[ima.length - 1]; }
        ...
        Num = Math.floor(Math.random() * (ima.length - (0 + counter)));
        temporary = ima[Num];
        if (localStorage.carSelected == 6 && Num == 2) {
            temporary = ima[Math.ceil(Math.random() * 3) * 4];
        }
    }
    counter = 1;
    return (temporary);
...
function setScores() {
    if (sessionStorage.highestScoreCasualMode > 0) {

    } else {
        sessionStorage.highestScoreCasualMode = 0;
    }

    if (localStorage.highestScoreCasualMode > 0) {

    } else {
        localStorage.highestScoreCasualMode = 0;
    }
}

setScores();

function givehighestScoreCasualMode() {
    highestScoreCasualMode = score;
    if (sessionStorage.highestScoreCasualMode < highestScoreCasualMode) {
        sessionStorage.highestScoreCasualMode = highestScoreCasualMode;
    }
    if (localStorage.highestScoreCasualMode < highestScoreCasualMode) {
        localStorage.highestScoreCasualMode = highestScoreCasualMode;
    }
}
...
function animate() {
    ...
    document.getElementById('scoreSession').innerHTML = "Sessions' Highest Score : " + sessionStorage.highestScoreCasualMode;
    document.getElementById('scoreAllTime').innerHTML = "Best Score : " + localStorage.highestScoreCasualMode;
    if (score > 0) {
...
function stopVideo() {
    ...
    let aboutVideo = document.getElementById('casualModeVideo');
    aboutVideo.pause();
